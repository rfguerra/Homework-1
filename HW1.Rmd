...
from data to derive theories. These models differ in assumption of parametric forms, number of observations, and flexibility. Mechanistic modeling initially assumes a parametric form, it's not exact to the collection of predictors, and it's not flexible with limited parameters. On the other hand, empirically-driven modeling does not assume predictors, requires more observations, and it is flexible. When many parameters are added in the mechanistic model, it derives 
...
flexibility. Thus, mechanistic modeling with few parameters is easier to interpret.

Bias-variance trade-off shows the inverse relationship between bias and variance for parameters. It is relevant to mechanistic and empirically-driven models because it derives changes in the models' flexibility/complexity. As the 
