...
Supervised learning demonstrates the existence of a response measurement y_i associated to individual measurement observations x_i, i= 1,...,n of the 
predictor. Predicts responses that are known.

Unsupervised learning demonstrates the existence of a collection of measurement observations x_i, i= 1,...,n but there is no associated measurement y_i. Results 
...
They attempt to predict a response Y with reducible and minimized error, without necessarily testing hypothesis.
...
it's not flexible with limited parameters. On the other hand, empirically-driven modeling does not assume predictors, requires more observations, and it is 
...
response Y from given data with minimal error. It does not attempt to derive hypothesis testing because there is no specific question in the statement 
...
Most of cars have an mpg to the left of 35. The scatter plot graph shows a proportional relationship between mpg in the city and on the highway. If mpg in 
